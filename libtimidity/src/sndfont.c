	...
	if ((sfrec.fd = timi_openfile(fname)) == NULL) {
        ...
        if (song->load_request_count < 128) {
            for (i=0; i < song->load_request_count; i++) {
                if (strcmp(song->load_requests[i], fname) == 0) {
                    return; // Already added this instrument, so return
                }
            }
            // Add instrument to load request list
            song->load_requests[song->load_request_count] = strdup(fname);
            song->load_request_count += 1;
        }
		return;
	}
	sfrec.fname = (char*)timi_calloc(strlen(fname)+1);
	...
	for (i = 0; i < sfinfo.nrpresets - 1; i++) {
		...
		if (bank == 128) {
			if (!song->drumset[preset]) {
				song->drumset[preset] = (MidToneBank*)timi_calloc(sizeof(MidToneBank));
				song->drumset[preset]->tone = (MidToneBankElement *) timi_calloc(128 * sizeof(MidToneBankElement));
			}
		} else {
			if (!song->tonebank[bank]) {
				song->tonebank[bank] = (MidToneBank*)timi_calloc(sizeof(MidToneBank));
...
MidInstrument *load_soundfont(MidSong *song, int order, int bank, int preset, int keynote)
{
	...
	if (sfrec.fd == NULL) {
		...
		if ((sfrec.fd = timi_openfile(sfrec.fname)) == NULL) {
            ...
            if (song->load_request_count < 128) {
                for (i=0; i < song->load_request_count; i++) {
                    if (strcmp(song->load_requests[i], sfrec.fname) == 0) {
                        return  NULL; // Already added this instrument, so return
                    }
                }
                // Add instrument to load request list
                song->load_requests[song->load_request_count] = strdup(sfrec.fname);
                song->load_request_count += 1;
            }
...
static MidInstrument *load_from_file(MidSong *song, SFInsts *rec, InstList *ip)
{
	...
	inst->sample = (MidSample*) timi_calloc(sizeof(MidSample)*ip->samples);
...
{
	SFExclude *rec;
	rec = (SFExclude*)timi_calloc(sizeof(SFExclude));
	rec->bank = bank;
	rec->preset = preset;
	rec->keynote = keynote;
	rec->next = sfexclude;
...
{
	SFOrder *rec;
	rec = (SFOrder*)timi_calloc(sizeof(SFOrder));
	rec->bank = bank;
	rec->preset = preset;
	rec->keynote = keynote;
	rec->order = order;
...
static void make_inst(MidSong *song, SFInsts *rec, Layer *lay, SFInfo *sf, int pr_idx, int in_idx, int order)
{
	...
	if (ip == NULL) {
		ip = (InstList*)timi_calloc(sizeof(InstList));
		ip->bank = bank;
		ip->preset = preset;
		ip->keynote = keynote;
		ip->order = order;
	...
	/* add a sample */
	sp = (SampleList*)timi_calloc(sizeof(SampleList));
	sp->next = ip->slist;
	ip->slist = sp;
	ip->samples++;
